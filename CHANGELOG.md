...
## [7.0.1] - 2026-01-17 - Flagship UX + security hardening

### Summary
- Добавлены единый словарь действий/алиасы параметров + “did you mean” подсказки: меньше промахов по `action`/полям.
- Улучшен UX долгих операций (exec/ssh): по умолчанию больше “follow/прогресса”, меньше ощущения “ничего не произошло”.
- Ошибки стали “тёплыми”: типизированные `ToolError` с подсказками и минимальными примерами вместо “InternalError”.
- Усилена безопасность вывода: агрессивная редактировка секретов; чтение текстовых артефактов в `base64` теперь блокируется по умолчанию (break-glass через `include_secrets=true` + allow-secret-export env).
- Улучшен project context summary: больше конкретных сигналов, меньше слепых догадок для агента.

### Compatibility
Patch-релиз: контракты совместимы. Возможны небольшие поведенческие отличия в дефолтном выводе (больше прогресса/статусов) и в редактировании/блокировке “подозрительных” данных по умолчанию.

## [7.0.0] - 2026-01-13 - JSON-first tool bus (breaking)

### Summary
- Ответы инструментов теперь **по умолчанию** возвращаются как строгий JSON envelope (machine-readable).
- Человекочитаемый `.context` остаётся, но уходит в артефакты (см. `artifact_uri_context`).
- Для каждого tool-call пишется `result.json` (строгий JSON) и возвращается `artifact_uri_json`.

### Compatibility
Breaking change: если у вас были скрипты/клиенты, которые парсили текстовый вывод, переключитесь на JSON envelope. Для human-view используйте `artifact_uri_context`.
