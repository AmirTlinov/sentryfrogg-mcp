...
test('SecretRefResolver selects vault profile by args.vault_profile_name', async () => {
    const calls = [];
    const resolver = new SecretRefResolver(loggerStub, validationStub, { async listProfiles() { return [{ name: 'ignored' }]; } }, { async kv2Get(profileName, ref) { calls.push({ profileName, ref }); return `v:${profileName}:${ref}`; } }, { async resolveContext() { return { target: { vault_profile: 'ignored' } }; } });
...
test('SecretRefResolver selects vault profile by project target', async () => {
    const calls = [];
    const resolver = new SecretRefResolver(loggerStub, validationStub, { async listProfiles() { return [{ name: 'ignored' }]; } }, { async kv2Get(profileName, ref) { calls.push({ profileName, ref }); return `v:${profileName}:${ref}`; } }, { async resolveContext() { return { target: { vault_profile: 'from-project' } }; } });
    const resolved = await resolver.resolveDeep({ token: 'ref:vault:kv2:kv/app#token' }, { project: 'demo', target: 'prod' });
...
test('SecretRefResolver selects vault profile when there is exactly one vault profile', async () => {
    const calls = [];
    const resolver = new SecretRefResolver(loggerStub, validationStub, { async listProfiles() { return [{ name: 'only' }]; } }, { async kv2Get(profileName, ref) { calls.push({ profileName, ref }); return `v:${profileName}:${ref}`; } }, { async resolveContext() { return null; } });
...
test('SecretRefResolver errors when multiple vault profiles exist and none is selected', async () => {
    const resolver = new SecretRefResolver(loggerStub, validationStub, { async listProfiles() { return [{ name: 'a' }, { name: 'b' }]; } }, { async kv2Get() { return 'nope'; } }, { async resolveContext() { return null; } });
    await assert.rejects(() => resolver.resolveDeep('ref:vault:kv2:secret/app#k', {}), /vault profile is required when multiple vault profiles exist/);
});
test('SecretRefResolver errors on unknown ref scheme', async () => {
    const resolver = new SecretRefResolver(loggerStub, validationStub, null, null, null);
    await assert.rejects(() => resolver.resolveDeep('ref:unknown:thing', {}), /Unknown secret ref scheme: unknown/);
});
test('SecretRefResolver caches identical vault refs within a single resolveDeep call', async () => {
    const calls = [];
    const resolver = new SecretRefResolver(loggerStub, validationStub, { async listProfiles() { return [{ name: 'only' }]; } }, { async kv2Get(profileName, ref) { calls.push({ profileName, ref }); return `v:${profileName}:${ref}`; } }, null);
