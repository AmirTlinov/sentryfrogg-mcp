...
test('repo git primitives enforce apply gating for write actions', async (t) => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'apply_patch', repo_root: repoRoot, patch: 'x' }), /apply=true is required/);
    await assert.rejects(() => manager.handleAction({ action: 'git_commit', repo_root: repoRoot, message: 'x' }), /apply=true is required/);
    await assert.rejects(() => manager.handleAction({ action: 'git_push', repo_root: repoRoot }), /apply=true is required/);
});
...
test('apply_patch rejects traversal paths inside patch headers', async (t) => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'apply_patch', repo_root: repoRoot, patch, apply: true }), /escapes repo_root/);
});
