...
test('LocalManager rejects when unsafe mode is disabled', async () => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'exec', command: 'noop' }), /Unsafe local tool is disabled/);
});
...
test('LocalManager fs_write refuses overwrite unless overwrite=true', async () => {
    ...
    try {
        ...
        await assert.rejects(() => manager.handleAction({ action: 'fs_write', path: filePath, content: 'two' }), /already exists/);
