...
test('repo.exec rejects commands outside allowlist and disallows whitespace in command', async (t) => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'exec', repo_root: repoRoot, command: 'bash', args: ['-c', 'echo hi'], apply: true }), /Command not allowed/);
    await assert.rejects(() => manager.handleAction({ action: 'exec', repo_root: repoRoot, command: 'git status', args: [], apply: true }), /single executable/);
});
...
test('repo.exec enforces sandboxed cwd and apply gating for mutating git subcommands', async (t) => {
    ...
    const manager = new RepoManager(loggerStub, securityStub, validationStub, null);
    await assert.rejects(() => manager.handleAction({ action: 'exec', repo_root: repoRoot, cwd: outside, command: 'git', args: ['status'] }), /escapes sandbox root/);
    await assert.rejects(() => manager.handleAction({ action: 'exec', repo_root: repoRoot, command: 'git', args: ['commit', '-m', 'x'] }), /requires apply=true/);
...
test('repo.exec allows read-only kubectl verbs without apply (including leading -n)', async () => {
    ...
    assert.throws(() => manager.ensureExecAllowed({ command: 'kubectl', argv: ['-n', 'default', 'annotate', 'pod', 'demo', 'a=b'], apply: false }), /requires apply=true/);
    assert.throws(() => manager.ensureExecAllowed({ command: 'kubectl', argv: ['rollout', 'undo', 'deployment/demo'], apply: false }), /requires apply=true/);
});
