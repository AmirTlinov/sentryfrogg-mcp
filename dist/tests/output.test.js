...
test('output.map throws when value is not an array and missing=error', () => {
    assert.throws(() => applyOutputTransform({ rows: { id: 1 } }, { path: 'rows', map: { pick: ['id'] } }), /expects an array/);
});
test('output.map returns [] when value is not an array and missing=empty', () => {
    const shaped = applyOutputTransform({ rows: { id: 1 } }, { path: 'rows', missing: 'empty', map: { pick: ['id'] } });
    assert.deepEqual(shaped, []);
});
test('output.default overrides missing behavior and still participates in map', () => {
    const shaped = applyOutputTransform({ ok: true }, {
        path: 'rows',
        missing: 'null',
