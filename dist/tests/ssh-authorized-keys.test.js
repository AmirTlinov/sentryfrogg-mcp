...
test('authorized_keys_add rejects invalid key format', async () => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'authorized_keys_add', public_key_path: pubPath, profile_name: 'default' }), /invalid format/);
});
test('authorized_keys_add rejects multi-line key input', async () => {
    ...
    await assert.rejects(() => manager.handleAction({ action: 'authorized_keys_add', public_key_path: pubPath, profile_name: 'default' }), /single key line/);
});
test('authorized_keys_add requires public_key or public_key_path', async () => {
    const manager = new SSHManager(loggerStub, securityStub, validationStub, profileServiceStub());
    await assert.rejects(() => manager.handleAction({ action: 'authorized_keys_add', profile_name: 'default' }), /public_key or public_key_path is required/);
});
