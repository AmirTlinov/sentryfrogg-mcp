...
async function spillLargeValues(value, options, pathSegments = [], state = null) {
    ...
    if (Array.isArray(value)) {
        const next = await Promise.all(value.map((item, idx) => spillLargeValues(item, ctx, pathSegments.concat(String(idx)), runState)));
        return next;
    }
    const out = {};
...
class ToolExecutor {
    ...
    async execute(tool, args = {}) {
        ...
        if (presetName && this.presetService) {
            presetData = this.normalizePresetData(await this.presetService.resolvePreset(resolved.tool, presetName));
        }
