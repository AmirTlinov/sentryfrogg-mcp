...
class ServiceBootstrap {
    ...
    static async registerBaseServices() {
        ...
        // Profile сервис
        this.container.register('profileService', (logger, security) => new ProfileService(logger, security), {
            singleton: true,
            dependencies: ['logger', 'security']
        });
        // Vault client (KV v2)
        this.container.register('vaultClient', (logger, validation, profileService) => new VaultClient(logger, validation, profileService), {
            singleton: true,
            dependencies: ['logger', 'validation', 'profileService'],
        });
        // State сервис
        this.container.register('stateService', (logger) => new StateService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Project сервис
        this.container.register('projectService', (logger) => new ProjectService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Project resolver (project/target → контекст вызова)
        this.container.register('projectResolver', (validation, projectService, stateService) => new ProjectResolver(validation, projectService, stateService), {
            singleton: true,
            dependencies: ['validation', 'projectService', 'stateService'],
        });
        // SecretRef resolver (Vault/ENV refs)
        this.container.register('secretRefResolver', (logger, validation, profileService, vaultClient, projectResolver) => new SecretRefResolver(logger, validation, profileService, vaultClient, projectResolver), {
            singleton: true,
            dependencies: ['logger', 'validation', 'profileService', 'vaultClient', 'projectResolver'],
        });
        // Runbook сервис
        this.container.register('runbookService', (logger) => new RunbookService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Context сервис
        this.container.register('contextService', (logger, projectResolver) => new ContextService(logger, projectResolver), {
            singleton: true,
            dependencies: ['logger', 'projectResolver'],
        });
        // ContextSession сервис
        this.container.register('contextSessionService', (logger, contextService, projectResolver, profileService) => new ContextSessionService(logger, contextService, projectResolver, profileService), {
            singleton: true,
            dependencies: ['logger', 'contextService', 'projectResolver', 'profileService'],
        });
        // Policy service (GitOps hardening)
        this.container.register('policyService', (logger, validation, stateService) => new PolicyService(logger, validation, stateService), {
            singleton: true,
            dependencies: ['logger', 'validation', 'stateService'],
        });
        // Capability сервис
        this.container.register('capabilityService', (logger, security) => new CapabilityService(logger, security), {
            singleton: true,
            dependencies: ['logger', 'security'],
        });
        // Evidence сервис
        this.container.register('evidenceService', (logger, security) => new EvidenceService(logger, security), {
            singleton: true,
            dependencies: ['logger', 'security'],
        });
        // Alias сервис
        this.container.register('aliasService', (logger) => new AliasService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Preset сервис
        this.container.register('presetService', (logger) => new PresetService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Audit сервис
        this.container.register('auditService', (logger) => new AuditService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Cache сервис
        this.container.register('cacheService', (logger) => new CacheService(logger), {
            singleton: true,
            dependencies: ['logger'],
        });
        // Workspace сервис
        this.container.register('workspaceService', (logger, contextService, contextSessionService, projectResolver, profileService, runbookService, capabilityService, projectService, aliasService, presetService, stateService) => new WorkspaceService(logger, contextService, contextSessionService, projectResolver, profileService, runbookService, capabilityService, projectService, aliasService, presetService, stateService), {
            singleton: true,
            dependencies: [
                'logger',
                'contextService',
    ...
    static async registerManagers() {
        ...
        // PostgreSQL Manager
        this.container.register('postgresqlManager', (logger, validation, profileService, projectResolver, secretRefResolver) => new PostgreSQLManager(logger, validation, profileService, projectResolver, secretRefResolver), {
            singleton: true,
            dependencies: ['logger', 'validation', 'profileService', 'projectResolver', 'secretRefResolver']
        });
        // SSH Manager
        this.container.register('sshManager', (logger, security, validation, profileService, projectResolver, secretRefResolver, jobService) => new SSHManager(logger, security, validation, profileService, projectResolver, secretRefResolver, jobService), {
            singleton: true,
        ...
        this.container.register('jobManager', (logger, validation, jobService, sshManager) => new JobManager(logger, validation, jobService, { sshManager }), {
            singleton: true,
            dependencies: ['logger', 'validation', 'jobService', 'sshManager'],
        });
        // API Manager
        this.container.register('apiManager', (logger, security, validation, profileService, cacheService, projectResolver, secretRefResolver) => new APIManager(logger, security, validation, profileService, cacheService, { projectResolver, secretRefResolver }), {
            singleton: true,
        ...
        if (isUnsafeLocalEnabled()) {
            this.container.register('localManager', (logger, validation) => new LocalManager(logger, validation, { enabled: true }), {
                singleton: true,
                dependencies: ['logger', 'validation'],
            });
        }
        // Repo Manager (safe-by-default)
        this.container.register('repoManager', (logger, security, validation, projectResolver, policyService) => new RepoManager(logger, security, validation, projectResolver, policyService), {
            singleton: true,
            dependencies: ['logger', 'security', 'validation', 'projectResolver', 'policyService'],
        });
        // Artifact Manager
        this.container.register('artifactManager', (logger, validation) => new ArtifactManager(logger, validation), {
            singleton: true,
            dependencies: ['logger', 'validation'],
        });
        // State Manager
        this.container.register('stateManager', (logger, stateService) => new StateManager(logger, stateService), {
            singleton: true,
            dependencies: ['logger', 'stateService'],
        });
        // Project Manager
        this.container.register('projectManager', (logger, validation, projectService, stateService) => new ProjectManager(logger, validation, projectService, stateService), {
            singleton: true,
            dependencies: ['logger', 'validation', 'projectService', 'stateService'],
        });
        // Env Manager
        this.container.register('envManager', (logger, validation, profileService, sshManager, projectResolver, secretRefResolver) => new EnvManager(logger, validation, profileService, sshManager, projectResolver, secretRefResolver), {
            singleton: true,
            dependencies: ['logger', 'validation', 'profileService', 'sshManager', 'projectResolver', 'secretRefResolver'],
        });
        // Vault Manager
        this.container.register('vaultManager', (logger, validation, profileService, vaultClient) => new VaultManager(logger, validation, profileService, vaultClient), {
            singleton: true,
            dependencies: ['logger', 'validation', 'profileService', 'vaultClient'],
        });
        // Context Manager
        this.container.register('contextManager', (logger, validation, contextService) => new ContextManager(logger, validation, contextService), {
            singleton: true,
            dependencies: ['logger', 'validation', 'contextService'],
        });
        // Capability Manager
        this.container.register('capabilityManager', (logger, security, validation, capabilityService, contextService) => new CapabilityManager(logger, security, validation, capabilityService, contextService), {
            singleton: true,
            dependencies: ['logger', 'security', 'validation', 'capabilityService', 'contextService'],
        });
        // Evidence Manager
        this.container.register('evidenceManager', (logger, security, validation, evidenceService) => new EvidenceManager(logger, security, validation, evidenceService), {
            singleton: true,
            dependencies: ['logger', 'security', 'validation', 'evidenceService'],
        });
        // Tool executor
        this.container.register('toolExecutor', (logger, stateService, aliasService, presetService, auditService, postgresqlManager, sshManager, jobManager, apiManager, repoManager, artifactManager, stateManager, projectManager, envManager, vaultManager, contextManager, capabilityManager, evidenceManager, aliasManager, presetManager, auditManager, pipelineManager) => new ToolExecutor(logger, stateService, aliasService, presetService, auditService, {
        ...
        // Runbook Manager
        this.container.register('runbookManager', (logger, runbookService, stateService, toolExecutor) => new RunbookManager(logger, runbookService, stateService, toolExecutor), {
            singleton: true,
            dependencies: ['logger', 'runbookService', 'stateService', 'toolExecutor'],
        });
        // Intent Manager
        this.container.register('intentManager', (logger, security, validation, capabilityService, runbookManager, evidenceService, projectResolver, contextService, policyService) => new IntentManager(logger, security, validation, capabilityService, runbookManager, evidenceService, projectResolver, contextService, policyService), {
            singleton: true,
            dependencies: [
                'logger',
                'security',
        ...
        // Workspace Manager
        this.container.register('workspaceManager', (logger, validation, workspaceService, runbookManager, intentManager, sshManager) => new WorkspaceManager(logger, validation, workspaceService, runbookManager, intentManager, sshManager), {
            singleton: true,
            dependencies: ['logger', 'validation', 'workspaceService', 'runbookManager', 'intentManager', 'sshManager'],
        });
        // Alias Manager
        this.container.register('aliasManager', (logger, aliasService) => new AliasManager(logger, aliasService), {
            singleton: true,
            dependencies: ['logger', 'aliasService'],
        });
        // Preset Manager
        this.container.register('presetManager', (logger, presetService) => new PresetManager(logger, presetService), {
            singleton: true,
            dependencies: ['logger', 'presetService'],
        });
        // Audit Manager
        this.container.register('auditManager', (logger, auditService) => new AuditManager(logger, auditService), {
            singleton: true,
            dependencies: ['logger', 'auditService'],
        });
        // Pipeline Manager
        this.container.register('pipelineManager', (logger, validation, apiManager, sshManager, postgresqlManager, cacheService, auditService, projectResolver) => new PipelineManager(logger, validation, apiManager, sshManager, postgresqlManager, cacheService, auditService, projectResolver), {
            singleton: true,
