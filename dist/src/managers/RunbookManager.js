...
class RunbookManager {
    ...
    async executeStep(step, stepKey, context, options) {
        ...
        if (retryConfig && typeof retryConfig === 'object' && retryConfig.max_attempts !== undefined) {
            const maxAttempts = Math.min(asPositiveInt(retryConfig.max_attempts, 1), MAX_RETRY_ATTEMPTS);
            const delayMs = Math.min(asNonNegativeInt(retryConfig.delay_ms ?? retryConfig.base_delay_ms, 0), MAX_RETRY_DELAY_MS);
