...
function resolveMaxCaptureBytes() {
    const fromEnv = readPositiveInt(process.env.SENTRYFROGG_PIPELINE_MAX_CAPTURE_BYTES
    ...
    async openHttpStream(httpArgs, cacheArgs, trace) {
        ...
        if (cachePolicy.enabled && this.cacheService && cacheKey) {
            ...
            if (cached) {
                ...
                const captured = await this.captureStreamToArtifact(createReadStream(cached.file_path), trace, { prefix: 'http-body' });
                return {
        ...
        if (!stream) {
            ...
            const captured = await this.captureStreamToArtifact(Readable.from(buffer), trace, { prefix: 'http-body' });
            return {
                stream: captured.stream,
                artifact: captured.artifact,
        ...
        if (cachePolicy.enabled && this.cacheService && cacheKey) {
            ...
            const captured = await this.captureStreamToArtifact(createReadStream(this.cacheService.dataPath(cacheKey)), trace, { prefix: 'http-body' });
            return {
                stream: captured.stream,
                artifact: captured.artifact,
                cache: { hit: false, key: cacheKey },
