...
async function fetchFn(...args) {
    ...
    if (!fetchPromise) {
        fetchPromise = Promise.resolve().then(() => __importStar(require('node-fetch'))).then((mod) => mod.default);
    }
...
class APIManager {
    ...
    resolveMaxCaptureBytes() {
        const fromEnv = readPositiveInt(process.env.SENTRYFROGG_API_MAX_CAPTURE_BYTES
