...
const { buildToolCallFileRef, resolveContextRepoRoot, writeBinaryArtifact, } = require('../utils/artifacts');
...
class RepoManager {
    ...
    async writeArtifact(filename, buffer, { traceId, spanId, truncated } = {}) {
        ...
        const artifact = await writeBinaryArtifact(contextRoot, buildToolCallFileRef({ traceId, spanId, filename }), buffer);
        return {
            uri: artifact.uri,
            rel: artifact.rel,
            path: artifact.path,
    ...
    async renderPlain({ repoRoot, overlayAbs, maxBytes }) {
        ...
        const combined = buffers.length === 1
            ? buffers[0]
            : Buffer.concat(buffers.flatMap((buf, idx) => (idx === 0 ? [buf] : [sep, buf])), total);
    ...
    async render(args) {
        ...
        const imagesRef = await this.writeArtifact('images.json', Buffer.from(`${JSON.stringify(extractedImages, null, 2)}\n`, 'utf8'), { traceId, spanId, truncated: false });
    ...
    async exec(args) {
        ...
        try {
            ...
            const stdoutArtifact = contextRoot && stdoutBuffer.length
                ? await writeBinaryArtifact(contextRoot, buildToolCallFileRef({ traceId, spanId, filename: 'stdout.log' }), stdoutBuffer)
                : null;
            const stderrArtifact = contextRoot && stderrBuffer.length
                ? await writeBinaryArtifact(contextRoot, buildToolCallFileRef({ traceId, spanId, filename: 'stderr.log' }), stderrBuffer)
                : null;
