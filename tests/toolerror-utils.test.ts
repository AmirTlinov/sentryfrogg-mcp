...
test('utils: sandbox traversal returns ToolError.denied', async () => {
  ...
  await assert.rejects(
    () => resolveSandboxPath(root, '../outside.txt'),
    (error) => ToolError.isToolError(error) && error.kind === 'denied' && error.code === 'PATH_ESCAPES_SANDBOX'
  );
});
...
test('utils: artifacts path escape returns ToolError.denied', async (t) => {
  ...
  assert.throws(
    () => resolveArtifactPath(root, '../evil.txt'),
    (error) => ToolError.isToolError(error) && error.kind === 'denied' && error.code === 'ARTIFACT_PATH_ESCAPES_ROOT'
  );
});

test('utils: dataPath required missing returns ToolError.invalidParams', () => {
  assert.throws(
    () => getPathValue({ ok: true }, 'missing.path', { required: true }),
    (error) => ToolError.isToolError(error) && error.kind === 'invalid_params' && /not found/i.test(error.message)
  );
});

test('utils: runbookDsl parse errors return ToolError.invalidParams', () => {
  assert.throws(
    () => parseRunbookDsl('wat something'),
    (error) => ToolError.isToolError(error) && error.kind === 'invalid_params'
  );
});
